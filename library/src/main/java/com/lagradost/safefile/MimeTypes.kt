...
object MimeTypes {
    fun fromMimeToExt(mime: String?): String? {
        return MimeTypeMap.getSingleton().getExtensionFromMimeType(mime ?: return null)
    }

    fun fromExtToMime(ext: String?): String? {
        return MimeTypeMap.getSingleton().getMimeTypeFromExtension(ext ?: return null)
    }

    // this is outdated and does not reflect how android treats mimetypes :pensive:
    /*
    fun fromMimeToExt(mime: String): Array<String>? {
        return internalMap[mime.lowercase()]
    }

    fun fromExtToMime(ext: String): ArrayList<String>? {
        return internalReverseMap[ext.lowercase()]
    }

    private val internalReverseMap: HashMap<String, ArrayList<String>> by lazy {
        val out: HashMap<String, ArrayList<String>> = hashMapOf()

        for ((key, values) in internalMap) {
            for (value in values) {
                val current = (out[value] ?: arrayListOf())
                current.add(key)
                out[value] = current
            }
        }
    ...
    private val internalMap: HashMap<String, Array<String>> = hashMapOf(
        ...
        "application/vnd.sqlite3" to arrayOf(
            "db",
            "sqlite",
            "sqlite3",
            "db-wal",
        ...
        "application/x-sqlite3" to arrayOf(
            "db",
            "sqlite",
            "sqlite3",
            "db-wal",
        ...
        "application/vnd.visio" to arrayOf(
            "vsd",
            "vss",
            "vst",
            "vsw",
        ...
        "application/x-director" to arrayOf(
            "cct",
            "cst",
            "cxt",
            "dcr",
